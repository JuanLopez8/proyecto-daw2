...
class Musica_model extends CI_Model
{
    ...
    {
        $musica = R::load('musica', $id);
        foreach($musica->ownMusicafavoritoList as $favorito){
        $f=R::load('musicafavorito', $favorito->id);
        R::trash($f);
        }
        foreach($musica->ownMusicaestadoList as $estado){
            $e=R::load('musicaestado', $estado->id);
            R::trash($e);
            }
        foreach($musica->ownMusicacomentarioList as $comentario){
            $c=R::load('musicacomentario', $comentario->id);
            R::trash($c);
        }
        foreach($musica->ownMusicavaloracionList as $valoracion){
            $v=R::load('musicavaloracion', $valoracion->id);
            R::trash($v);
        }
        R::trash($musica);
    }
...
public function deleteComentario($id){
    $bean=R::load('musicacomentario', $id);
    R::trash($bean);
}
...
//Cargar la música recomendada para un usuario
public function getMusicasRecomendadas($id){
    $beans= R::getAssoc("Select * from musicavaloracion where usuario_id=? and valor>4", [$id]);
    $claves=array_keys($beans);
    $i=0;
    $datos=array();
    foreach($beans as $bean){
        $dato=["id"=> $claves[$i], "valor"=>$bean["valor"], "usuario_id"=>$bean["usuario_id"], "musica_id"=>$bean["musica_id"]];
        $datos[$i]=$dato;
        $i++;
    }
    $ok=$beans!=null;
    $musicasMasValoradas=array();
    $musicasValoradas=array();
    $musicasCasiRecomendadas=array();
    $musicasRecomendadas=[];
    $generos=array();
    $generosUsados=array();
    if($ok){
        foreach($datos as $msc){
            $musica=R::findOne('musica', "id=?", [$msc['musica_id']]);
            if(!in_array($musica->genero, $generosUsados)){
                array_push($generosUsados, $musica->genero);
            }
        }
        foreach($generosUsados as $gen){
            $musicasMasValoradas=array_merge($musicasMasValoradas, R::findAll('musica', 'genero=?', [$gen]));
        }
        $valor=0;
        if($musicasMasValoradas!=null){
            foreach($musicasMasValoradas as $musica){
                $musicasCasiRecomendadas=array_merge($musicasCasiRecomendadas, R::findAll('musicavaloracion', 'musica_id=?', [$musica->id]));
                
            }
            foreach($musicasCasiRecomendadas as $msc){
                $valor=$this->getValoracionMedia($msc->id);
                if($valor>=4){
                    array_push($musicasRecomendadas, $msc);
                }
            }
            }

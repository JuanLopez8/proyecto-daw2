...
class Libro_model extends CI_Model
{
    ...
    //ACTUALIZAR UN LIBRO
    public function update($id, $nombre_nuevo, $descripcion_nuevo, $genero_nuevo, $numero_paginas_nuevo, $autor_nuevo, $editorial_nuevo, $fecha_nuevo, $idUsu, $imagen)
    {
        $ok = false;
    ...
    {
        $libro = R::load('libro', $id);
        foreach($libro->ownLibrofavoritoList as $favorito){
        $f=R::load('librofavorito', $favorito->id);
        R::trash($f);
        }
        foreach($libro->ownLibroestadoList as $estado){
            $e=R::load('libroestado', $estado->id);
            R::trash($e);
            }
        foreach($libro->ownLibrocomentarioList as $comentario){
            $c=R::load('librocomentario', $comentario->id);
            R::trash($c);
        }
        foreach($libro->ownLibrovaloracionList as $valoracion){
            $v=R::load('librovaloracion', $valoracion->id);
            R::trash($v);
        }
        R::trash($libro);
    }
...
public function deleteComentario($id){
    $bean=R::load('librocomentario', $id);
    R::trash($bean);
}
...
//Cargar los libros recomendados para un usuario
public function getLibrosRecomendados($id){
    $beans= R::getAssoc("Select * from librovaloracion where usuario_id=? and valor>4", [$id]);
    $claves=array_keys($beans);
    $i=0;
    $datos=array();
    foreach($beans as $bean){
        $dato=["id"=> $claves[$i], "valor"=>$bean["valor"], "usuario_id"=>$bean["usuario_id"], "libro_id"=>$bean["libro_id"]];
        $datos[$i]=$dato;
        $i++;
    }
    $ok=$beans!=null;
    $librosMasValorados=array();
    $librosValoradas=array();
    $librosCasiRecomendados=array();
    $librosRecomendados=[];
    $generos=array();
    $generosUsados=array();
    if($ok){
        foreach($datos as $lib){
            $libro=R::findOne('libro', "id=?", [$lib['libro_id']]);
            if(!in_array($libro->genero, $generosUsados)){
                array_push($generosUsados, $libro->genero);
            }
        }
        foreach($generosUsados as $gen){
            $librosMasValorados=array_merge($librosMasValorados, R::findAll('libro', 'genero=?', [$gen]));
        }
        $valor=0;
        if($librosMasValorados!=null){
            foreach($librosMasValorados as $libro){
                $librosCasiRecomendados=array_merge($librosCasiRecomendados, R::findAll('librovaloracion', 'libro_id=?', [$libro->id]));
            }
            foreach($librosCasiRecomendados as $libro){
                $valor=$this->getValoracionMedia($libro->id);
                if($valor>=4){
                    array_push($librosRecomendados, $libro);
                }
            }
            }

<link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/detalles.css"/>
	<div class="fondo" style='background-image: url("<?=base_url().$pelicula->ruta_caratula?>")'>
	<?php if(isset($_SESSION['rol']) && $_SESSION['rol']=="administrador"):?>
		...
		<form action="<?=base_url()?>pelicula/delete" method="POST">
		<input type="hidden" name="id" value="<?=$pelicula->id?>"/>
		<input type="submit" class="btn btn-danger" value="Eliminar película"/>
		</form>
		<?php endif;?>
		<img src="<?=base_url().$pelicula->ruta_caratula?>" onerror="this.src='<?=base_url()?>assets/img/404.png';" alt="portada" class="caratula">
...
<?php endif;?>
	...
	<?php if(isset($_SESSION['id'])):?>
			<div id="contIframe">
				<iframe  width="70%" height="70%" src="https://www.youtube.com/embed?&color=white&listType=search&list=<?=$pelicula->nombre?> trailer" frameborder="0" allowfullscreen="" style="position:absolute;"></iframe>
			</div>
				<?php endif;?>
	<!-- Dejar un comentario -->
	<div id="comentarios">
	...
	<?php if(isset($_SESSION['id']) && $_SESSION['id']!=null):?>
		<form action="" method="POST">
			...
			<input type="button" onclick="enviarForm('pelicula')" name="Publicar" class="btn-info" style="border-radius:5px;" value="Publicar"/>
		</form>
	<?php endif;?>
		...
		<div class="comentarios">
			...
			<?php if(isset($_SESSION['rol']) && $_SESSION['rol']=="administrador"):?>
			<form action="<?=base_url()?>pelicula/deleteComentario" method="POST">

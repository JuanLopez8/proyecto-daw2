...
class Usuario extends CI_Controller
{
    ...
    //Ayudar al usuario a recuperar su contraseña
    public function ayudaLogin(){
        frame($this, 'usuario/ayudaLogin');
    }
    ...
    public function cambiarImagenPost(){
        ...
        if($id!=null){
            $this->load->model('usuario_model');
            $data['res']=$this->usuario_model->cambiarImagen($id, $imagenes);
            if($data['res']=="ok"){
                header("location: ".base_url());
            }
            else{
                frame($this, "usuario/cambiarImagenError", $data);
            }
    }
}
    ...
    public function updatePost()
    {
        ...
        if ($correo_nuevo != null && $nombreUsuario_nuevo != null) {
            $this->load->model('usuario_model');
            $ok = $this->usuario_model->update($id, $correo_nuevo, $nombreUsuario_nuevo, $nombre_nuevo, $apellidos_nuevo, $descripcion_nuevo, $fnac_nuevo);
            if ($ok) {
                session_start();
                session_destroy();
    ...
    public function deleteAdmin()
    {
        ...
        if ($id != null) {
            $this->load->model('usuario_model');
            $this->usuario_model->delete($id);
            header("location: ".base_url()."usuario/listarTodo");
        }
    }
